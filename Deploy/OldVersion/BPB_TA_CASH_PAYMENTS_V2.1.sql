/***************************************************************************************************************/
-- Име          : Янко Янков
-- Дата и час   : 17.05.2022
-- Задача       : Task 276959 (v2.1)
...
begin
	...
	if @LogResultTable = 1 select @DEAL_NUM as [DEAL_NUM], 0 as [TAX_UNCOLLECTED], @CorrAccCurrency as [CCY], @CorrAccount as [ACC], @DealRowID as [DEAL_ROW_ID]
	;
	if @LogResultTable = 1 select [DEAL_NUM], [TAX_UNCOLLECTED_SUM], [CURRENCY], [UI_CORR_ACCOUNT], [ROW_ID] from dbo.[DEALS_CORR_TA] [D] where [D].[ROW_ID] = @TBL_ROW_ID
	;
...
begin
	...
	begin
		...
		declare @CUSTOMER_ROLE_TYPE varchar(8) = N''
		;
	...
	if @Rows <= 0 and @Save_TA_Cinditions = 1
	begin
...
begin
	...
	declare @LogTraceInfo int = 0, @LogResultTable int = 0,	@LogSqlCondition int = 0
	...
	if @LogSqlCondition = 1 select * from dbo.[#TBL_SQL_CONDITIONS] with(nolock);
