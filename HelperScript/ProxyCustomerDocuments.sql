...
SELECT [T].*
FROM dbo.[AGR_CASH_PAYMENTS_DEALS_WITH_OTHER_TAX_ACCOUNT] [T]
LEFT OUTER JOIN dbo.[AGR_CASH_PAYMENTS_DEALS] [D]
...
SELECT [D].*
FROM dbo.[AGR_CASH_PAYMENTS_DEALS] [D]
LEFT OUTER JOIN dbo.[AGR_CASH_PAYMENTS_DEALS_WITH_OTHER_TAX_ACCOUNT] [T]
	ON [D].DEAL_NUM = [T].DEAL_NUM
WHERE [D].DEAL_HAS_OTHER_TAX_ACC = 1
...
SELECT *
FROM dbo.[AGR_CASH_PAYMENTS_CUSTOMERS] [C]
WHERE EXISTS 
(
	SELECT *
	FROM dbo.[AGR_CASH_PAYMENTS_CUSTOMERS_ARE_PROXIES] [P]
...
(
	...
	WHERE	[PS].[REPRESENTATIVE_CUSTOMER_ID] = [C].[CUSTOMER_ID]
		...
		AND [PS].[CUSTOMER_ROLE_TYPE] IN ( 2 /* NM622: 2 - Пълномощник */ )
) 
go
...
INNER JOIN BPB_VCSBank_Online.dbo.[CUSTOMERS_RIGHTS_AND_LIMITS] [CRL] WITH(NOLOCK) 
	...
	AND	[CRL].[CUSTOMER_ROLE_TYPE] in (1, 3) /* NM622 (client roles): 1 - Титуляр, 3 - Законен представител, ... */
...
DECLARE @DealType int = 1 /* Razp deals 1 */
	...
	,	@RoleProxy int = 2 /* NM622 (client roles): 1 - Титуляр, 2- Пълномощник; 3 - Законен представител, ... */
	,	@StsDeActivated int	= dbo.SETBIT(cast(0 as binary(4)), 12, 1)	/* #define STS_LIMIT_DEACTIVATED 12 (DeActivated) */
;
SELECT	[D].[DEAL_TYPE]
		,	[D].[DEAL_NUM]
...
into #AGR_CASH_PAYMENTS_DEALS_ACTIVE_PROXY_CUSTOMERS 
FROM dbo.[AGR_CASH_PAYMENTS_DEALS] [D] WITH(NOLOCK)
INNER JOIN BPB_VCSBank_Online.dbo.[CUSTOMERS_RIGHTS_AND_LIMITS] [CRL] WITH(NOLOCK) 
	ON	[CRL].[DEAL_TYPE]	= @DealType 
	AND	[CRL].[DEAL_NUM]	= [D].[DEAL_NUM] 
...
from #AGR_CASH_PAYMENTS_DEALS_ACTIVE_PROXY_CUSTOMERS 
group by [DEAL_NUM]
having count(*) > 1
order by cnt desc
go
...
SELECT * FROM AGR_CASH_PAYMENTS_DEALS_ACTIVE_PROXY_CUSTOMERS
...
INNER JOIN AGR_CASH_PAYMENTS_DEALS_ACTIVE_PROXY_CUSTOMERS [X]
	ON [P].CUST_ID = [X].REPRESENTATIVE_CUSTOMER_ID
LEFT OUTER JOIN  AGR_CASH_PAYMENTS_CUSTOMERS [C]
...
SELECT * FROM AGR_CASH_PAYMENTS_CUSTOMERS
