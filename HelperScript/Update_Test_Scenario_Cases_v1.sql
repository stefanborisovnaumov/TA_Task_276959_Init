...
--		,	[C].[CUST_CND_HAVE_CREDIT]					AS [HAVE_CREDIT]
	...
	SELECT * FROM SYS.TABLES WHERE NAME LIKE 'AGR_CASH_PAYMENTS%'
	GO

	SELECT * FROM DT015_CUSTOMERS_ACTIONS_TA
	WHERE ROW_ID =	100019

	SELECT * FROM [AGR_CASH_PAYMENTS_CUSTOMERS_WITH_MANY_CLIENT_CODES]
	GO

	SELECT	[X].[CUSTOMER_CHARACTERISTIC]
		,	[X].*, [F].*
	FROM #TBL_TA_CONDITIONS [F]
	CROSS APPLY (
		SELECT TOP(1) 
				[D].DEAL_TYPE
			,	[D].DEAL_NUM
			,	[C].[CUSTOMER_ID]
			--,	[MANY_CC].[CUSTOMER_ID] AS [MANY_CC_CUST_ID]
			,	[C].CLIENT_BIRTH_DATE
			,	[C].[CUSTOMER_CHARACTERISTIC]
		FROM dbo.[AGR_CASH_PAYMENTS_DEALS] [D] WITH(NOLOCK)
		INNER JOIN dbo.[AGR_CASH_PAYMENTS_CUSTOMERS] [C] WITH(NOLOCK)
			ON	[D].[CUSTOMER_ID] = [C].[CUSTOMER_ID]
			AND [F].[UI_STD_DOG_CODE] = [D].DEAL_STANDART_CONTRACT_NUMBER
			AND	[F].[DB_CLIENT_TYPE_DT300] = [C].[CLIENT_TYPE_DT300_CODE]

		INNER JOIN dbo.[AGR_CASH_PAYMENTS_CUSTOMERS_WITH_DUBL_EGFN] [DUBL] WITH(NOLOCK)
			ON	[DUBL].[CUSTOMER_ID] = [C].[CUSTOMER_ID]
			AND [DUBL].IS_ORIGINAL_EGFN	= (CASE	WHEN [F].[IS_SERVICE] = 1 THEN 1 
												WHEN [F].[IS_SERVICE] = 2 THEN 0 END) 

		--LEFT OUTER JOIN dbo.[AGR_CASH_PAYMENTS_CUSTOMERS_WITH_MANY_CLIENT_CODES] [MANY_CC] WITH(NOLOCK)
		--	ON	[MANY_CC].[CUSTOMER_ID] = [C].[CUSTOMER_ID]
		--	AND [DUBL].IS_ORIGINAL_EGFN	= (CASE	WHEN [F].[CUST_CND_IS_SERVICE] = 1 THEN 1 
		--										WHEN [F].[CUST_CND_IS_SERVICE] = 2 THEN 0 END) 
		--	AND (CASE WHEN [F].[CUST_CND_UNIFIED] IN ( -1, 0 ) THEN 0 ELSE 1 END)
		--		= (CASE WHEN [MANY_CC].[CUSTOMER_ID] IS NULL THEN 0 ELSE 1 END )
	) [X]
	WHERE	[F].[IS_SERVICE] IN (1, 2)
		--AND (CASE WHEN [F].[CUST_CND_UNIFIED] IN ( -1, 0 ) THEN 0 ELSE 1 END)
		--	= (CASE WHEN [X].[MANY_CC_CUST_ID] IS NULL THEN 0 ELSE 1 END )
	ORDER BY [F].[UI_STD_DOG_CODE]
	GO
